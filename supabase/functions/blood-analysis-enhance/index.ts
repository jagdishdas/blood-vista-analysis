...
serve(async (req) => {
    ...
    try {
        ...
        console.log(`Blood analysis enhancement request: ${testType}, ${results.length} parameters`);
        ...
        return new Response(JSON.stringify({
            error: 'No AI provider configured. Please set GEMINI_API_KEY or OPENAI_API_KEY in Supabase secrets.'
        }), {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
        ...
        return new Response(
            JSON.stringify({
                ...
                details: 'Please check Supabase function logs for more information.'
            }),
            {
                status: 500,
                headers: { ...corsHeaders, "Content-Type": "application/json" },
